...
import omni.common.Omni as CommonOmni
import omni.common.data.repository.MobileReaderDriverRepository as CommonMobileReaderDriverRepo
...
class Omni(omniApi: OmniApi) : CommonOmni(omniApi) {
    ...
    companion object {
        ...
        fun initialize(params: Map<String, Any>, completion: () -> Unit, error: (OmniException) -> Unit) {
            // Init the API
            val omniApi = OmniApi()
            omniApi.environment = params.get("environment") as? OmniApi.Environment
