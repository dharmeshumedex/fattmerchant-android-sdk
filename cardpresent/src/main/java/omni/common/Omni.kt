...
import omni.common.data.models.Invoice
...
open class Omni(var omniApi: OmniApi) {
    ...
    val uncaughtHandlerException = CoroutineExceptionHandler { _, excpetion ->

    }

    fun exceptionHandler(errorHandler: (OmniException) -> Unit): CoroutineExceptionHandler {
        return CoroutineExceptionHandler { _, exception ->
            if (exception is OmniException) {
                errorHandler(exception)
            } else {
                errorHandler(OmniException(exception.message ?: "", ""))
            }
        }
    }
    ...
    fun initialize(args: Map<String, Any>, completion: () -> Unit, error: (OmniException) -> Unit) {
        coroutineScope.launch(exceptionHandler(error)) {
            ...
            try {
                ...
                error(OmniException("Could not initialize mobile reader", e.message))
            }
        }
    }
